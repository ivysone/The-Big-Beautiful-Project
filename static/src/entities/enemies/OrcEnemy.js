...
export class OrcEnemy extends Phaser.Physics.Matter.Sprite {
  /**
  ...
  constructor(scene, x, y, deps) {
    super(scene.matter.world, x, y, 'orc');
    ...
    const mainBody = Bodies.rectangle(0, 0, 20, 43, { label: 'orcBody' });
    const footSensor = Bodies.rectangle(0, 24, 16, 4, { isSensor: true, label: 'orcFoot' });
  ...
  static preload(scene) {
    scene.load.spritesheet('orc','/static/assets/Enemies/mixed/orc.png', { 
      frameWidth: 64, 
      frameHeight: 64 
    });
  }
  ...
  initAnimations(scene) {
    ...
    if (!scene.anims.exists('orc_idle')) {
      scene.anims.create({
        key: 'orc_idle',
        frames: scene.anims.generateFrameNumbers('orc', { start: 0, end: 4}),
        frameRate: 8,
        repeat: -1
      });
    }

    if (!scene.anims.exists('orc_run')) {
      scene.anims.create({
        key: 'orc_run',
    ...
    if (!scene.anims.exists('orc_attack')) {
      scene.anims.create({
        key: 'orc_attack',
    ...
    if (!scene.anims.exists('orc_death')) {
      scene.anims.create({
        key: 'orc_death',
        frames: scene.anims.generateFrameNumbers('orc', { start: 24, end: 30}),
        frameRate: 8,
        repeat: 0
      });
    }
  ...
  preUpdate(time, delta) {
    ...
    if (!this.isAggro) {
      ...
      if (dist >= this.deaggroRange) {
        ...
        this.path = null;
        this.setVelocityX(0);
        if (this.anims.currentAnim?.key !== 'orc_idle') this.play('orc_idle', true);
        return;
      }
    }
    ...
    if (this.anims.currentAnim?.key !== 'orc_run') this.play('orc_run', true);
  }
  ...
  startMeleeAttack() {
    ...
    this.once(Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + 'orc_attack', () => {
      this.isAttacking = false;
      this.setMeleeActive(false);
      if (!this.isDead) this.play('orc_idle', true);
    });
  }
  ...
  die() {
    ...
    this.once(Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + 'orc_death', () => {
      this.destroy();
    });
  }
