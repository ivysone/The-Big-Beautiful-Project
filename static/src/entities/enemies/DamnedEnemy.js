...
export class DamnedEnemy extends Phaser.Physics.Matter.Sprite {
  /**
  ...
  constructor(scene, x, y, deps) {
    super(scene.matter.world, x, y, 'damned');
    ...
    const mainBody = Bodies.rectangle(0, 0, 20, 43, { label: 'damnedBody' });
    const footSensor = Bodies.rectangle(0, 24, 16, 4, { isSensor: true, label: 'damnedFoot' });
    ...
    this.play('damned_idle');
  }

  // ASSET LOADING
  static preload(scene) {
    scene.load.spritesheet('damned','/static/assets/Enemies/damned/damnedMale.png', { 
      frameWidth: 64, 
      frameHeight: 64 
    });
  }
  ...
  initAnimations(scene) {
    ...
    if (!scene.anims.exists('damned_idle')) {
      scene.anims.create({
        key: 'damned_idle',
        frames: scene.anims.generateFrameNumbers('damned', { start: 0, end: 4}),
        frameRate: 8,
        repeat: -1
      });
    }

    if (!scene.anims.exists('damned_run')) {
      scene.anims.create({
        key: 'damned_run',
        frames: scene.anims.generateFrameNumbers('damned', { start: 8, end: 15}),
        frameRate: 8,
        repeat: -1
      });
    }

    if (!scene.anims.exists('damned_attack')) {
      scene.anims.create({
        key: 'damned_attack',
        frames: scene.anims.generateFrameNumbers('damned', { start: 16, end: 21}),
        frameRate: 8,
        repeat: 0
      });
    }

    if (!scene.anims.exists('damned_death')) {
      scene.anims.create({
        key: 'damned_death',
        frames: scene.anims.generateFrameNumbers('damned', { start: 24, end: 29}),
        frameRate: 8,
        repeat: 0
      });
    }
  ...
  preUpdate(time, delta) {
    ...
    if (!this.isAggro) {
      ...
      if (dist >= this.deaggroRange) {
        ...
        this.path = null;
        this.setVelocityX(0);
        if (this.anims.currentAnim?.key !== 'damned_idle') this.play('damned_idle', true);
        return;
      }
    }
    ...
    if (this.anims.currentAnim?.key !== 'damned_run') this.play('damned_run', true);
  }
  ...
  startMeleeAttack() {
    ...
    this.once(Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + 'damned_attack', () => {
      this.isAttacking = false;
      this.setMeleeActive(false);
      if (!this.isDead) this.play('damned_idle', true);
    });
  }
  ...
  die() {
    ...
    this.once(Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + 'damned_death', () => {
      this.destroy();
    });
  }
