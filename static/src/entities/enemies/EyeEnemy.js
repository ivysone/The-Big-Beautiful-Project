...
export class EyeEnemy extends Phaser.Physics.Matter.Sprite {
  /**
  ...
  constructor(scene, x, y, deps) {
    super(scene.matter.world, x, y, "eye");
    ...
    const mainBody = Bodies.rectangle(0, 0, 20, 20, { label: "eyeBody" });
  ...
  static preload(scene) {
    scene.load.spritesheet("eye", "/static/assets/Enemies/mixed/eye.png", {
      frameWidth: 64,
      frameHeight: 64,
    });
  }
  ...
  initAnimations(scene) {
    if (!scene.anims.exists("eye_idle")) {
      scene.anims.create({
        key: "eye_idle",
        frames: scene.anims.generateFrameNumbers("eye", { start: 0, end: 4 }),
        frameRate: 8,
        repeat: -1,
      });
    }
    ...
    if (!scene.anims.exists("eye_death")) {
      scene.anims.create({
        key: "eye_death",
        frames: scene.anims.generateFrameNumbers("eye", { start: 6, end: 10 }),
        frameRate: 5,
        repeat: 0,
      });
    }
  ...
  preUpdate(time, delta) {
    ...
    if (!this.isAggro) {
      ...
      if (distToHover >= this.deaggroRange) {
        ...
        if (this.anims.currentAnim?.key !== "eye_idle") this.play("eye_idle", true);
        return;
      }
    }
  ...
  startMeleeAttack() {
    ...
    this.once(
        Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + "eye_attack",
        () => {
  ...
  die() {
    ...
    this.play("eye_death");
    this.once(Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + "eye_death", () => {
      this.destroy();
    });
  }
